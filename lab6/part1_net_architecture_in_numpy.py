...
def zad2_two_layer_net(student_id):
    ...
    class SimpleTwoLayerNetwork:
        ...
        def forward(self, x_data):
            # tmp = self.hidden_layer.forward(x_data)
            # inspect_data(x, tmp, 6)
            # plot_data(x, tmp, plot_xy_range=[-1, 2])
    ...
    model.hidden_layer.W[:, 0] = [0.957, 0.287]      # wagi neuronu h1
    model.hidden_layer.W[:, 1] = [-0.88, -0.48]      # wagi neuronu h2
    model.hidden_layer.b[:] = [-0.287, 1.056]         # biasy neuronów h1 i h2
    # model.output_layer.W[:, 0] = [1, 1]      # wagi neuronu wyjściowego
    # model.output_layer.b[:] = [-1]         # bias neuronu wyjściowego
    model.output_layer.W[:, 0] = [0.707, 0.707]      # wagi neuronu wyjściowego
    model.output_layer.b[:] = [-1.06]
...
if __name__ == '__main__':
    ...
    student_id = 197808         # Twój numer indeksu, np. 102247
