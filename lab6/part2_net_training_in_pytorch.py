...
def training(model, x, y):
    ...
    for _ in range(n_steps):
        ...
        rand_idx = torch.randint(x.shape[0], (minibatch_size,))
        x_batch = torch.stack([x[idx] for idx in rand_idx])
        y_batch = torch.FloatTensor([y[idx] for idx in rand_idx]).unsqueeze(1)
